...
function getVerdictShort(verdict: string): string {
  ...
  if (verdict === 'IDLENESS_LIMIT_EXCEEDED') return 'ILE'
  ...
  ctx.command("cf <username>")
    .action(async ({ session }, username) => {
      try {
        ...
        const ratingHistory = await ctx.http.get(`https://codeforces.com/api/user.rating?handle=${username}`)
        const lastSubmission = await ctx.http.get(`https://codeforces.com/api/user.status?handle=${username}&from=1&count=1`)
        
        // if (response?.status !== 'OK' || submissionList?.status !== 'OK' || ratingHistory?.status !== 'OK' || lastSubmission?.status !== 'OK') {
        ...
        const ratingChanges: RatingChange[] = ratingHistory.result
        const lastSub: Submission = lastSubmission.result[0]
        
        // 获取最后一次 rating 变化
        const lastRatingChange = ratingChanges.length > 0 ? ratingChanges[ratingChanges.length - 1] : null
        const ratingDiff = lastRatingChange ? lastRatingChange.newRating - lastRatingChange.oldRating : 0
        ...
        ctx2d.font = '26px'
        ctx2d.fillStyle = '#ffffff'
        ctx2d.fillText(`Friend Of: ${user.friendOfCount}`, 30, 310)
        ctx2d.fillText(`Max Rating: ${user.maxRating}`, 30, 340)
        ctx2d.fillText(`Solved Problems: ${solvedCount}`, 30, 370)

        // 绘制最后一次 rating 变化
        if (lastRatingChange) {
          const ratingChangeText = `Last Rating Change: ${ratingDiff > 0 ? '+' : ''}${ratingDiff}`
          ctx2d.fillText(ratingChangeText, 30, 400)
        }

        // 绘制最后一次提交状态
        if (lastSub) {
          const verdictText = `Last Submission Status: ${getVerdictShort(lastSub.verdict)}`
          ctx2d.fillText(verdictText, 30, 430)
        }
